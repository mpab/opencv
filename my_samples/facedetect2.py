...
if __name__ == '__main__':
    ...
    cascade = []
    for cfn in cascade_fn:
        cascade.append(cv2.CascadeClassifier(cfn))

    nested = cv2.CascadeClassifier(nested_fn)

    #cam = create_capture('synth:bg=monkey-race-wide.jpg:noise=0.05', None)
    cam = create_capture('synth:bg=monkey-race-wide.jpg')
    ...
    while True:
        ...
        t = clock()
        rects = detect(gray, cascade[cascade_idx])

        algo_info = 'algorithm = {}:{}'.format(cascade_idx, cascade_fn[cascade_idx])
